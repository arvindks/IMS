...
%           2. Cross Validation grid search with exact solve
%
...
%  Modification:
%  05/24/2016 - use x = X(:) as the true signal rather than x=X_noise(:)
...
result_bic = zeros(n_repeat, n_finer+2, n_miss_pct);
...
for miss_pct_idx = 1:n_miss_pct
    ...
    for repeat_idx = 1:n_repeat
        ...
        X_miss= missing_pattern(X_noise, miss_pct,...
                                'pattern','random');        
        %x = X_noise(:);
        x = X(:);
        ...
        for finer_idx =1:n_finer
            ...
            col_gamma_list = exp(linspace(-9, 1,...
                   init_grid_n+(init_grid_n-1)*(2^(finer_idx-1)-1)));
            tic;
            [X_complete,gamma_best,bic_best,mse_grid] = bmc_cv(X_miss, ...
                                      row_gamma_list,col_gamma_list,...
                                      row_laplacian,col_laplacian,'K',5);
                                  
            result_runtime(repeat_idx, finer_idx,miss_pct_idx) = toc;
            result_bic(repeat_idx, finer_idx,miss_pct_idx) = bic_best;
        ...
        % fminunc
        tic;
        [X_complete,~,bic_best] = bmc_qn(X_miss,[0 0],...
                row_laplacian,col_laplacian);
        result_runtime(repeat_idx, n_finer+1,miss_pct_idx) = toc;
        result_bic(repeat_idx, n_finer+1,miss_pct_idx) = bic_best;
        z = X_complete(:);
        ...
        result_bic(repeat_idx, n_finer+2,miss_pct_idx) = bic_best;
        z = X_complete(:);
...
%% Save results
FileName = ['S:\Documents\BMC\results\','Figure4_experiments_',...
